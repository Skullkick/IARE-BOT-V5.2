...
- **Fix Applied:**
  ...
  5. Implemented interactive high-compression preview & 3-button confirmation workflow:
     ...
     - Displays a 2-row action layout: Row 1 has `[ Confirm ]` (submits document) and `[ Resend ]` (retains experiment info and waits for replacement PDF); Row 2 has full-width `[ Cancel ]` (purges all PDFs, wipes staged SQLite data via `tdatabase.delete_lab_upload_data`, and resets status).
